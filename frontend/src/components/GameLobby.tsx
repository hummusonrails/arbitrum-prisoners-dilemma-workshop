...
const GameLobby: React.FC<GameLobbyProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      {/* Create Deal Form */}
      {showCreateForm && (
        <div className="bg-gray-900 p-6 rounded-lg mb-8 border-2 border-teal-600 shadow-lg shadow-teal-900/50">
          <h3 className="text-2xl font-bold mb-4 text-teal-400">CREATE NEW DEAL</h3>
          <div className="flex flex-col sm:flex-row gap-4">
            <input
              ...
              className="flex-1 p-3 rounded bg-gray-800 text-white border-2 border-gray-700 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/50 outline-none transition-all"
            />
            <div className="flex flex-col gap-2 justify-center sm:flex-row sm:items-center">
              <button
                onClick={handleCreateCell}
                disabled={loading}
                className="bg-gradient-to-r from-teal-600 to-emerald-700 hover:from-teal-500 hover:to-emerald-600 text-white px-8 py-4 rounded-xl font-black text-lg tracking-wider transition-all duration-300 transform hover:scale-110 hover:shadow-2xl border-2 border-teal-400/50 hover:border-teal-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none"
              >
      ...
      {activeCells.length > 0 && (
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-teal-400 mb-4">ACTIVE DEALS</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {activeCells.map((cell) => (
              <div
                key={cell.id}
                className="bg-gray-800/50 border border-teal-500/30 rounded-lg p-4 hover:bg-gray-700/50 transition-colors cursor-pointer"
                onClick={() => onEnterCell(cell.id)}
              >
                <div className="flex justify-between items-center mb-2">
                  <span className="text-lg font-medium text-white">Deal #{cell.id}</span>
      ...
      <div className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-bold text-teal-400">AVAILABLE DEALS</h2>
          <span className="px-3 py-1 bg-gray-800 text-teal-400 text-sm font-mono rounded-full border border-teal-600">
            {openCells.length} DEAL{openCells.length !== 1 ? 'S' : ''} AVAILABLE
          </span>
        </div>
        ...
        {openCells.length > 0 ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {openCells.map((cell) => (
              <div key={cell.id} className="bg-gray-900 p-5 rounded-xl border-2 border-gray-700 hover:border-teal-600 transition-all hover:shadow-lg hover:shadow-teal-900/30">
                <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-800">
                  <span className="font-mono text-teal-400">DEAL #{cell.id}</span>
                  <span className="px-2 py-1 bg-gray-800 text-cyan-400 text-xs font-bold rounded">
                ...
                <div className="space-y-2 mb-5">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-teal-300">{formatEther(cell.stake)} ETH</span>
                  </div>
                </div>
                {canJoinCell(cell) ? (
                  <button
                    ...
                    onMouseOver={(e) => {
                      ...
                      e.currentTarget.style.borderColor = '#115e59';
                  ...
                  <button
                    onClick={() => onEnterCell(cell.id)}
                    disabled
                    className="w-full py-3 group relative bg-gradient-to-r from-teal-600 to-teal-700 text-white font-black text-lg rounded-xl tracking-wider border-2 border-teal-400/50 opacity-50 cursor-not-allowed"
                  >
                    ENTER DEAL
                  </button>
                ) : (
          ...
          <div className="text-center py-10 bg-gray-900/50 rounded-xl border-2 border-dashed border-gray-800">
            <p className="text-gray-500 text-lg">No open deals found.</p>
            <button
      ...
      {activeCells.length > 0 && (
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-3xl font-bold text-teal-400">YOUR ACTIVE DEALS</h2>
            <span className="px-3 py-1 bg-gray-800 text-teal-400 text-sm font-mono rounded-full border border-teal-600">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {activeCells.map((cell) => {
              ...
              return (
                <div key={cell.id} className="relative bg-gray-900 p-5 rounded-xl border-2 border-cyan-600 hover:shadow-lg hover:shadow-cyan-900/30 transition-all">
                  <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-800">
                    <span className="font-mono text-teal-400">DEAL #{cell.id}</span>
                  ...
                  <button
                    ...
                    className="w-full py-3 px-6 rounded-xl font-bold text-lg transition-colors duration-200
                      bg-gradient-to-r from-teal-400 to-cyan-600 text-gray-900 shadow-md
                      disabled:bg-gray-300 disabled:text-gray-400 disabled:cursor-not-allowed
                      hover:from-teal-500 hover:to-cyan-700 focus:outline-none focus:ring-2 focus:ring-teal-400"
                  >
                    ENTER DEAL
                  </button>
                </div>
      ...
      {completedCells.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-teal-400">RECENTLY COMPLETED</h2>
            <span className="px-3 py-1 bg-gray-800 text-gray-400 text-sm font-mono rounded-full border border-gray-700">
              LAST {Math.min(3, completedCells.length)} DEALS
            </span>
          </div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              return (
                <div key={cell.id} className="bg-gray-900/50 p-5 rounded-xl border-2 border-gray-800 hover:border-gray-700 transition-colors">
                  ...
                  <button
                    onClick={() => onEnterCell(cell.id)}
                    className="w-full py-3 px-6 rounded-xl font-bold text-lg transition-colors duration-200
                      bg-gradient-to-r from-emerald-400 to-teal-600 text-gray-900 shadow-md
                      disabled:bg-gray-300 disabled:text-gray-400 disabled:cursor-not-allowed
                      hover:from-emerald-500 hover:to-teal-700 focus:outline-none focus:ring-2 focus:ring-emerald-400"
                  >
                    ENTER DEAL
                  </button>
                </div>
