...
docker run --rm --name nitro-dev -p 8547:8547 "${TARGET_IMAGE}" --dev --http.addr 0.0.0.0 --http.api=net,web3,eth,debug --http.corsdomain="*" &
...
cast send -r $RPC --private-key $PRIVATE_KEY 0x0000000000000000000000000000000000000070 'setL2BaseFee(uint256)' 0x3b9aca00

# Fund the CREATE2 factory address before deploying
cast send --rpc-url $RPC --private-key $PRIVATE_KEY --value 1ether --gas-price 1000000000 0x3fab184622dc19b6109349b94811493bf2a45362

# Deploy CREATE2 factory
echo "Deploying the CREATE2 factory"
cast send --rpc-url $RPC --private-key $PRIVATE_KEY --value "1 ether" --gas-price 1000000000 0x3fab184622dc19b6109349b94811493bf2a45362
